...
def correct_datetime(value):
    try:
        # Jeśli wartość jest None, zwróć None, aby SQL mógł użyć NULL
        if pd.isna(value) or value is None:
            return None
        # Konwertuj wartość na timestamp (milisekundy do sekund)
        timestamp = int(float(value) / 1000)
        # Konwertuj timestamp na obiekt datetime
        corrected_datetime = datetime.fromtimestamp(timestamp)
        # Zwróć w formacie datetimeSQL
        return corrected_datetime.strftime('%Y-%m-%d %H:%M:%S')
    except (ValueError, OverflowError, TypeError):
        # Zwróć None zamiast domyślnej daty
        return None
...
def import_csv_to_mysql(table_name, file_path):
...
def main():
    ...
    try:
        print("Connecting:")
        conn = pymysql.connect(**MYSQL_CONFIG, connect_timeout=30)
    ...
    for table in table_order:
        import_csv_to_mysql(table, FILES[table])
